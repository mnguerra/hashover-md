<?php
	...
	if ($page_title == 'yes') {
		$js_title = '<script type="text/javascript">if (document.title != "") { document.write(" '.$text['published_on'].' \"" + document.title + "\"") };</script>';
		$js_title = (isset($_GET['pagetitle'])) ? ' '.$text['published_on'].' "' . $_GET['pagetitle'] . '"' : $js_title;
	}
...
<?php
	...
	if ($cmt_count == "1") {
		echo "\t" . '<br><b class="cmtfont">' . $text['zero_cmt'] . '.</b>' . PHP_EOL;;
	} else {
		echo "\t" . '<br><b class="cmtfont">' . $text['showing_cmts'] . ' ' . $script = ($cmt_count == "1") ? '0 comentaris:</b>' . PHP_EOL : display_count() . ':</b>' . PHP_EOL;
	}

	// Display comments in case there is anyone
	if (!empty($show_cmt)) {
...
?>
	...
	<form id="comment_form" name="comment_form" action="<?php echo $root_dir; ?>comments.php" method="post">
<?php if ($icons == 'yes') { ?>
...
Comentaris fets amb <a rel="nofollow" href="https://github.com/jacobwb/hashover">HashOver</a> amb <a rel="nofollow" href="https://github.com/mnguerra/hashover">adaptacions pròpies</a>.
</p>
